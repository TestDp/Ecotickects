...
class EventosRepositorio
{
                ...
                foreach ($EdEvento->localidad as $localidad ){
                    if($EdEvento->idPrecioBoleta[$indPrecio] == 0)
                    {
                        ...
                        if($EdEvento->esPromo[$indPrecio]==1){
                            ...
                            $precioBoletaPromo ->cantidad = $EdEvento->CantidadCod[$indPrecio];
                        ...
                        }else if($EdEvento->esConvenio[$indPrecio]==1){
                                $PrecioBoleta ->Codigo = $EdEvento->Codigo[$indPrecio];
                                $PrecioBoleta ->esConvenio = $EdEvento->esConvenio[$indPrecio];
                        }
                        $PrecioBoleta ->save();
                    }else{
                        ...
                        if($EdEvento->esConvenio[$indPrecio]==1){
                            $PrecioBoleta ->Codigo = $EdEvento->Codigo[$indPrecio];
                            $PrecioBoleta ->esConvenio = $EdEvento->esConvenio[$indPrecio];
                        }
                        $PrecioBoleta ->save();
    ...
    public function calcularDescPrecioBoleta($precioBase,$porcentaje){
        return $this->CalcularPrecioBoletaPromo($precioBase,$porcentaje);
    }

    public function obtenerBoletaPromo($idEvento, $codigo)
    {
        $idsLocalidadesDisponibles =$this->obtenerIdLocalidadDisponible($idEvento);
    ...
    {
        return $localidadDisponible = DB::select("CALL usp_ObtenerIdLocalidadesDisponible($idEvento)");
    }
    ...
    public function obtenerEventoEditar($idEvento)
    {
        ...
        $evento->preciosBoletas = DB::table('Tbl_PreciosBoletas')
            ...
            ->select('Tbl_PreciosBoletas.*','ph.esCodigoPromo', 'ph.Codigo as codigoH', 'ph.Porcentaje', 'ph.id as idHijo','ph.cantidad as cantidadCod')
            ...
            ->join(DB::raw('(SELECT e.Nombre_Evento as Nombre_Evento,(case when p.MediosDePago_id = 2 then 1 else 0 end) 
                ...
                function($join)
                {
                    ...
                    ->on('resul.EsTC','=','Tbl_ConfiguracionXSedes.EsTC')
                        ->where('Tbl_ConfiguracionXSedes.EsActivo','=',1);
                })
