...
class AsistentesController extends Controller
{
        ...
        if ($respuesta == 'true') {
			...
			*/
            Mail::send('Email/correo', ['ElementosArray' => $ElementosArray], function ($msj) use ($evento, $ccUser, $correoElectronico, $correoSaliente, $nombreEvento) {
				...
				$qr = base64_encode(\QrCode::format('png')->merge('../../public_html/img/iconoPequeno.png')->size(280)->generate($nombreEvento . ' - CC - ' . $ccUser . 'ECOTICKETS'));
    ...
    public function GenerarQRS(Request $formRegistro)
    {
        ...
        Mail::send('Email/correo', ['ElementosArray' => $ElementosArray], function ($msj) use ($correoElectronico, $correoSaliente, $nombreEvento,$evento,$pinUser) {
            ...
            $msj->to($correoElectronico);
            $msj->bcc('soporteecotickets@gmail.com');
            $qr = base64_encode(\QrCode::format('png')->merge('../../public_html/img/iconoPequeno.png')->size(280)->generate($nombreEvento . ' - CC - ' . $pinUser . 'ECOTICKETS'));
            $ElementosArray = array('evento' => $evento, 'qr' => $qr);
            //preguntamos si el directorio existe
            if (!file_exists(storage_path('app') . '/boletas/'.$evento->id)) {
                mkdir(storage_path('app') . '/boletas/'.$evento->id, 0777, true);
            ...
            if ($estadoVenta == 4 && $verficarFirma == 1) {
                ...
                Mail::send('Email/correo', ['ElementosArray' => $ElementosArray], function ($msj) use ($pinesImagenes, $correoElectronico, $correoSaliente, $nombreEvento, $evento) {
                    ...
                    foreach ($pinesImagenes as $pin) {
                        $qr = base64_encode(\QrCode::format('png')->merge('../../public_html/img/iconoPequeno.png')->size(280)->generate($nombreEvento . ' - CC - ' . $pin->PinBoleta . 'ECOTICKETS'));
                        $ElementosArray = array('evento' => $evento, 'qr' => $qr);
